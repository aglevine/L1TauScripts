...
drawUCTRlx = False #draw relaxed curves
...
if "H2" in eff_ntuple:
        ...
        Hthresh = 4
elif "H5" in eff_ntuple:
	Hthresh = 5
else:
        "missing Hthresh"
        sys.exit()
selection = argv[2]
turnOffTauVeto = float(argv[3])
LIso=3
LSB=50
...
if Cmp4x4 == False:
        saveWhere = 'DiTauplots/PtOrdering/2DDiTauEff'+str(Ethresh)+'H'+str(Hthresh)+selection+'RlxVeto'+str(TauVetoRlx)+'OffIso'+str(turnOffIso)+'OffVeto'+str(turnOffTauVeto)+'Iso'+str(ISOTHRESHOLD)+'L1pTThresh'+str(l1ptVal)
if drawL1Rlx == True:
...
def make_res_nrml(ntuple_rlx_4x8,ntuple_iso_4x8,ntuple_rlx_4x4,ntuple_iso_4x4,reco,l1,l1g,binning,cutPtVarg='l1gPt',cutPtVar='l1Pt',cutPt=l1ptVal,isoCut='',isoCut4x4='',filename='',extraCut='',extraCutb='',setLOG=False):
 ...
 if selection == "NoVeto":
        ...
        vetoCut = vetoCut + "&&(l1gTauVetoNeighbor[0] == 0 || l1gPt[0] > "+str(2000)+")&&(l1gTauVetoNeighbor[1]==0 || l1gPt[1] > "+str(2000)+")&&((l1gTauVeto[0] == 0  || l1gPt[0] > " +str(turnOffTauVeto) + ")&&(l1gTauVeto[1] == 0  || l1gPt[1] > " +str(turnOffTauVeto) + "))"
...
def effi_histo(ntuple,variable,cut,binning,denom,title,leg,color,marker,logg):
 ...
 if drawUCTIso_byhand:
 	isoStr = "ISO < %0.2f"%(ISOTHRESHOLD)
 else:
	isoStr = "No ISO"
 if "TauVeto" in selection:
	selectionStr = "Tau Veto Selection"
 else:
 	selectionStr = "No Tau Veto Selection"
