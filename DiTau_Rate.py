...
ratePlots = True # which curves to draw on rate and efficiency plots
...
##################
rate_ntuple = argv[1]
if "E2" in rate_ntuple:
        Ethresh = 2
elif "E3" in rate_ntuple:
        Ethresh = 3
elif "E4" in rate_ntuple:
        Ethresh = 4
elif "E5" in rate_ntuple:
	Ethresh = 5
else:
        "missing Ethresh"
        sys.exit()
if "H2" in rate_ntuple:
        Hthresh = 2
elif "H3" in rate_ntuple:
        Hthresh = 3
elif "H4" in rate_ntuple:
        Hthresh = 4
elif "H5" in rate_ntuple:
	Hthresh = 5
else:
        "missing Hthresh"
        sys.exit()
...
LIso=3
LSB=50
l1ptVal=float(argv[4])
recoPtVal=0
ISOTHRESHOLD=float(argv[5])
...
saveWhere = 'plots/Fixed4x4Flag_Jul17/Tau4x8vs4x4'+selection+'RlxVeto'+str(TauVetoRlx)+'Apply4x4TauVeto'+str(Veto4x4)+'OffIso'+str(turnOffIso)+'OffVeto'+str(turnOffTauVeto)+'Iso'+str(ISOTHRESHOLD)+'L1pTVal'+str(l1ptVal)
#saveWhere = 'plots/4x8vs4x4E3H3/Tau4x8RateTest.png'
if Cmp4x4 == False:
	saveWhere = 'DiTauplots/Aug29Taus/DiTauRateE'+str(Ethresh)+'H'+str(Hthresh)+selection+'RlxVeto'+str(TauVetoRlx)+'OffIso'+str(turnOffIso)+'OffVeto'+str(turnOffTauVeto)+'Iso'+str(ISOTHRESHOLD)+'L1pTThresh'+str(l1ptVal)
if drawL1Rlx == True:
...
if selection == "NoVeto":
       ...
       vetoCut = "(l1gTauVetoNeighbor[0] == 0)&&(l1gTauVetoNeighbor[1] == 0)"
       vetoCut4x4 = "1"
elif selection == "TauVeto":
       vetoCut = "(l1gTauVeto[0]==0||pt[0]>"+str(turnOffTauVeto)+")&&(l1gTauVeto[1]==0||pt[1]>"+str(turnOffTauVeto)+")"
       vetoCut4x4 = vetoCut
elif selection == "TauVetoAndNeighbor":
       vetoCut = '(l1gTauVeto[0]==0||pt[0]>'+str(turnOffTauVeto)+')&&(l1gTauVetoNeighbor[0]==0||pt[0]>'+str(turnOffTauVeto)+')&&(l1gTauVeto[1]==0||pt[1]>'+str(turnOffTauVeto)+')&&(l1gTauVetoNeighbor[1]==0||pt[1]>'+str(turnOffTauVeto)+')'
	#||pt[0]>'+str(turnOffTauVeto)+')'
       vetoCut4x4 = "l1gTauVeto[0]==0||pt[0]>"+str(turnOffTauVeto)
elif selection == "AllTauVeto":
       vetoCut4x4 = "(l1gTauVeto[0] == 0 || pt[0] > " +str(turnOffTauVeto) + ")"
       vetoCut = "(associated4x4TauE[0] == 0&&associated4x4TauSE[0] == 0&&associated4x4TauS[0] == 0&&associated4x4TauSW[0] == 0&&associated4x4TauW[0] == 0&&associated4x4TauNW[0] == 0&&associated4x4TauN[0] == 0&&associated4x4TauNE[0] == 0&&(l1gTauVeto[0] == 0 || pt[0] > " +str(turnOffTauVeto) + "))"
else:
...
if Cmp4x4 == True:
	#rate_ntuple4x4 = '4x4Taus/tau_rate_4x4_E3H3.root'
	rate_ntuple4x4 = 'tau_rate_4x4_E3H3_fixdo4x4Flag_July17.root'
...
name=''
if drawUCTIso or drawUCTRlx or drawUCTRlxTauVeto or drawUCTIso_byhand: name+='_UCT_'
...
def make_rate_plot(
 ...
 tex.SetTextSize(0.07)
 tex.SetTextAlign(11)
 legend = ROOT.TLegend(0.4,0.5,0.89,0.89,'','brNDC')
...
# Current Relaxed
 if drawL1Rlx_:
  cut_L1_rlx='(eta[0] > -2.5 && eta[0] < 2.5)&&(eta[1] > -2.5 && eta[1] < 2.5)'
...
# UCT Relaxed
 if drawUCTRlx_:
  if TauVetoRlx == False:
	cut_UCT_rlx='(eta[0] > -2.5 && eta[0] < 2.5)&&(eta[1] > -2.5 && eta[1] < 2.5)'
  ...
  h_UCT_rlx,max_UCT_rlx,vert_UCT_rlx,hor_UCT_rlx,uctrlxRateVal = rate_histo(
   ...
   scale,colorUR,22,legend,
   rlxLegendStr,
   log,line,ptLine,aUR,bUR)
 if drawUCTRlxTauVeto:
  cut_UCT_rlxTauVeto = extraCut
  rlxTauVetoLegendStr = 'UCT (4x8): Rlx'
  h_UCT_rlxTauVeto,max_UCT_rlxTauVeto,vert_UCT_rlxTauVeto,hor_UCT_rlxTauVeto,uctrlxTauVetoRateVal = rate_histo(
   ntuple_UCT_rlx,cut_UCT_rlxTauVeto,binning,L1G_CALIB_FACTOR,
   scale,ROOT.EColor.kGreen+4,22,legend,
   rlxTauVetoLegendStr,
   log,line,ptLine,aUR,bUR)  
...
# UCT Isolated
 ...
 if Cmp4x4:
  ...
  h_UCT_rlx4x4,max_UCT_rlx4x4,vert_UCT_rlx4x4,hor_UCT_rlx4x4,uctrlxRateVal4x4 = rate_histo(
   ...
   scale4x4,ROOT.EColor.kMagenta,22,legend,
   rlxLegendStr4x4,
   log,line,ptLine,aUR,bUR)
# UCT Rlx + Isolation by hand
 ...
 latexStrRlx = "Rlx 4x8 Rate (No Tau Veto):  %.2f kHz "%(uctrlxRateVal/1000)
 latexStrRlxTauVeto = "Rlx 4x8 Rate (Tau Veto):  %.2f kHz "%(uctrlxTauVetoRateVal/1000)
 latexStrIso = "Iso 4x8 Rate:  %.2f kHz "%(uctibhRateVal/1000)
 #latex.DrawLatex(0.8,0.65,latexStrRlx)
 #latex.DrawLatex(0.8,0.6,latexStrRlxTauVeto)
 #latex.DrawLatex(0.8,0.55, latexStrIso)
 if drawL1Rlx == True:
	latexStrL1Rlx = "Rlx L1 Rate: %.2f kHz "%(l1rlxRateVal/1000)
	latex.DrawLatex(0.8,0.55,latexStrL1Rlx)
 if Cmp4x4 == True:
...
# ptLine=20,
 ...
 isoCut='(( pt[0]>='+str(turnOffIso)+' && (jetPt[0]-pt[0])/pt[0]<100)||(pt[0]<'+str(turnOffIso)+'&&(jetPt[0]-pt[0])/pt[0]<'+str(ISOTHRESHOLD)+'))'+'&&(( pt[1]>='+str(turnOffIso)+' && (jetPt[1]-pt[1])/pt[1]<100)||(pt[1]<'+str(turnOffIso)+'&&(jetPt[1]-pt[1])/pt[1]<'+str(ISOTHRESHOLD)+'))',
 isoCut4x4='(( regionPt[0]>='+str(turnOffIso)+' && (jetPt[0]-regionPt[0])/regionPt[0]<100)||(regionPt[0]<'+str(turnOffIso)+'&&(jetPt[0]-regionPt[0])/regionPt[0]<'+str(ISOTHRESHOLD4x4)+'))',
 ...
 extraCut='(eta[0] > -2.5 && eta[0] < 2.5)&&(eta[1] > -2.5 && eta[1] < 2.5)&&'+vetoCut,
